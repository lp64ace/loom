...
class GHOST_TimerManager {
   public:
	...
	/**
	 * Checks all timer tasks to see if they are expired and fires them if
	 * needed. \param time: The current time. \return True if any timers were
	 * fired.
	 */
	bool fireTimers(uint64_t time);

	/**
	 * Checks this timer task to see if they are expired and fires them if
	 * needed. \param time: The current time. \param task: The timer task to
	 * check and optionally fire. \return True if the timer fired.
	 */
