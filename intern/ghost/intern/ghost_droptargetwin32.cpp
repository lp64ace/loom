												  ...
												  DWORD *pdw_effect)
{
	if (m_window->canAcceptDragOperation()) {
		...
		*pdw_effect = DROPEFFECT_NONE;
		/* XXX Uncomment to test drop. Drop will not be called if `pdw_effect ==
		 * DROPEFFECT_NONE`. */
...
GHOST_TDragnDropTypes GHOST_DropTargetWin32::getGhostType(
...
void *GHOST_DropTargetWin32::getDropDataAsFilenames(IDataObject *p_data_object)
{
	...
	if (p_data_object->QueryGetData(&fmtetc) == S_OK) {
		...
		if (p_data_object->GetData(&fmtetc, &stgmed) == S_OK) {
			...
			const unsigned int totfiles = ::DragQueryFileW(hdrop, -1, NULL, 0);
			if (totfiles) {
				str_array = (GHOST_TStringArray *)::malloc(
					sizeof(GHOST_TStringArray));
				str_array->count = 0;
				str_array->strings = (uint8_t **)::malloc(totfiles *
														  sizeof(uint8_t *));

				for (unsigned int nfile = 0; nfile < totfiles; nfile++) {
					WCHAR fpath[MAX_PATH];
					if (::DragQueryFileW(hdrop, nfile, fpath, MAX_PATH) > 0) {
						char *temp_path;
						if (!(temp_path = alloc_utf_8_from_16(fpath, 0))) {
							/* Just ignore paths that could not be converted
							 * verbatim. */
							continue;
						}
						str_array->strings[str_array->count++] = (uint8_t *)
							temp_path;
					}
				}
			}
...
void *GHOST_DropTargetWin32::getDropDataAsString(IDataObject *p_data_object)
{
	...
	if (p_data_object->QueryGetData(&fmtetc) == S_OK) {
		if (p_data_object->GetData(&fmtetc, &stgmed) == S_OK) {
			...
			if (tmp_string) {
				::strcpy_s(tmp_string, ::strlen(str) + 1, str);
			}
			/* Free memory. */
			::GlobalUnlock(stgmed.hGlobal);
			::ReleaseStgMedium(&stgmed);
...
int GHOST_DropTargetWin32::WideCharToANSI(LPCWSTR in, char *&out)
{
	...
	size = ::WideCharToMultiByte(
		CP_ACP,		/* System Default Codepage */
	...
	size = ::WideCharToMultiByte(
		CP_ACP, 0x00000400, in, -1, (LPSTR)out, size, NULL, NULL);
