...
public:
	...
	template<typename U , LOOM_ENABLE_IF ( ( is_span_convertible_pointer_v<U , _Tp> ) )> constexpr Span ( const U *start , size_t size ) : mData ( static_cast< const _Tp * >( start ) ) , mSize ( size ) {
	}

	/** Reference an initializer_list. Note that the data in the initializer_list is only valid
	 * until the expression containing it is fully computed.
	 *
	 * Do:
	...
	constexpr Span ( const std::initializer_list<_Tp> &list ) : Span ( list.begin ( ) , static_cast< size_t >( list.size ( ) ) ) {
	}

	constexpr Span ( const std::vector<_Tp> &vector ) : Span ( vector.data ( ) , static_cast< size_t >( vector.size ( ) ) ) {
	}

	template<std::size_t N> constexpr Span ( const std::array<_Tp , N> &array ) : Span ( array.data ( ) , N ) {
	}

	/** Support implicit conversions like the one below:
	 *   Span<T *> -> Span<const T *> */
	template<typename U , LOOM_ENABLE_IF ( ( is_span_convertible_pointer_v<U , _Tp> ) )> constexpr Span ( Span<U> span ) : mData ( static_cast< const _Tp * >( span.data ( ) ) ) , mSize ( span.size ( ) ) {
	}

	/** Returns a contiguous part of the array. This invokes undefined behavior when the start
	 * or size is negative. */
	constexpr Span Slice ( size_t start , size_t size ) const {
		const size_t new_size = std::max<size_t> ( 0 , std::min ( size , mSize - start ) );
	...
	/** Returns the pointer to the beginning of the referenced array. This may be nullptr when
	 * the size is zero. */
	...
	/** Access an element in the array. This invokes undefined behavior when the index is out
	 * of bounds. */
	...
	/** Does a constant time check to see if the pointer points to a value in the referenced
	 * array. Return true if it is, otherwise false. */
	...
	/** Return a reference to the first element in the array. This invokes undefined behavior
	 * when the array is empty. */
	constexpr const _Tp &First ( ) const {
		return mData [ 0 ];
	}

	/** Returns a reference to the nth last element. This invokes undefined behavior when the
	 * span is too short. */
	...
	/** Returns the element at the given index. If the index is out of range, return the
	 * fallback value. */
	...
	/** Check if the array contains duplicates. Does a linear search for every element. So the
	 * total running time is O(n^2). Only use this for small arrays. */
	constexpr bool HasDuplicates__linear_search ( ) const {
		/* The size should really be smaller than that. If it is not, the calling code
		 * should be changed. */
	...
	/** Returns true when this and the other array have an element in common. This should only
	 * be called on small arrays, because it has a running time of O(n*m) where n and m are the
	 * sizes of the arrays. */
	...
	/** Returns the index of the first occurrence of the given value. This invokes undefined
	 * behavior when the value is not in the array. */
	...
	/** Utility to make it more convenient to iterate over all indices that can be used with
	 * this array. */
...
public:
	...
	constexpr MutableSpan ( std::vector<_Tp> &vector ) : MutableSpan ( vector.data ( ) , vector.size ( ) ) {
	}

	template<std::size_t N> constexpr MutableSpan ( std::array<_Tp , N> &array ) : MutableSpan ( array.data ( ) , N ) {
	}
	 ...
	 */
	template<typename U , LOOM_ENABLE_IF ( ( is_span_convertible_pointer_v<U , _Tp> ) )> constexpr MutableSpan ( MutableSpan<U> span ) : mData ( static_cast< _Tp * >( span.data ( ) ) ) , mSize ( span.size ( ) ) {
	}
	...
	template<typename U , LOOM_ENABLE_IF ( ( is_span_convertible_pointer_v<_Tp , U> ) )> constexpr operator Span<U> ( ) const {
	...
	/** Replace a subset of all elements with the given value. This invokes undefined behavior
	 * when one of the indices is out of bounds. */
	...
	/** Returns a pointer to the beginning of the referenced array. This may be nullptr, when
	 * the size is zero. */
	...
	/** Returns a new MutableSpan that only contains the first n elements. This invokes
	 * undefined behavior when n is negative. */
	 ...
	 * due to implicit conversions. However, sometimes automatic type deduction needs some
	 * help. */
	...
	/** Copy all values from another span into this span. This invokes undefined behavior when
	 * the destination contains uninitialized data and T is not trivially copy constructible.
...
// This is defined here, because in `loomlib_index_range.hh` `Span` is not yet defined.
inline Span<size_t> IndexRange::AsSpan ( ) const {
	const size_t min_required_size = mStart + mSize;
	const size_t current_array_size = s_current_array_size.load ( std::memory_order_acquire );
	const size_t *current_array = s_current_array.load ( std::memory_order_acquire );
