...
GHOST_TSuccess GHOST_DisplayManagerWin32::getNumDisplays(
	uint8_t &numDisplays) const
{
...
GHOST_TSuccess GHOST_DisplayManagerWin32::getNumDisplaySettings(
	uint8_t display, int32_t &numSettings) const
{
	/* When you call #EnumDisplaySettings with #iModeNum set to zero, the
	 * operating system initializes and caches information about the display
	 * device. When you call #EnumDisplaySettings with #iModeNum set to a
	 * non-zero value, the function returns the information that was cached the
	 * last time the function was called with #iModeNum set to zero. */
...
GHOST_TSuccess GHOST_DisplayManagerWin32::getDisplaySetting(
	uint8_t display, int32_t index, GHOST_DisplaySetting &setting) const
{
	DISPLAY_DEVICE display_device;
	if (!get_dd(display, &display_device))
		return GHOST_kFailure;
	...
	if (::EnumDisplaySettings(display_device.DeviceName, index, &dm)) {
		 ...
		 * #dmDisplayFrequency member may return with the value 0 or 1. These
		 * values represent the display hardware's default refresh rate. This
		 * default rate is typically set by switches on a display card or
		 * computer motherboard, or by a configuration program that does not use
		 * Win32 display functions such as #ChangeDisplaySettings. */

		/* First, we tried to explicitly set the frequency to 60 if
		 * #EnumDisplaySettings returned 0 or 1 but this doesn't work since
		 * later on an exact match will be searched. And this will never happen
		 * if we change it to 60. Now we rely on the default hardware setting.
		 */
	...
	uint8_t display, const GHOST_DisplaySetting &setting)
{
	...
	switch (status) {
		...
		case DISP_CHANGE_RESTART:
			printf(
				"display change: The computer must be restarted in order for "
				"the graphics mode to "
				"work.\n");
			break;
		...
		case DISP_CHANGE_FAILED:
			printf(
				"display change: The display driver failed the specified "
				"graphics mode.\n");
			break;
		...
		case DISP_CHANGE_NOTUPDATED:
			printf(
				"display change: Windows NT: Unable to write settings to the "
				"registry.\n");
			break;
		default:
