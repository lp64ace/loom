 ...
 * In other words, Xtilt and Ytilt are components of a vector created by
 * projecting the pen's angle in 3D space vertically downwards on to the XY
 * plane
 * --Matt
 */
typedef enum {
...
typedef enum {
	 ...
	 * utilities, but we use the dependent Windows Pointer API. */
...
typedef struct GHOST_TabletData {
	...
	float Xtilt; /* range 0.0 (upright) to 1.0 (tilted fully against the tablet
					surface) */
...
#define GHOST_KEY_MODIFIER_NUM \
	((_GHOST_KEY_MODIFIER_MAX - _GHOST_KEY_MODIFIER_MIN) + 1)
...
#define GHOST_KEY_MODIFIER_CHECK(key) \
	(GHOST_KEY_MODIFIER_TO_INDEX(key) < GHOST_KEY_MODIFIER_NUM)
...
typedef enum {
	...
	GHOST_kGrabWrap,
	/**
	 * Hide the mouse while grabbing and restore the original location on
	 * release (used for number buttons and some other draggable UI elements).
	 */
	GHOST_kGrabHide,
...
typedef void (*GHOST_TimerProcPtr)(struct GHOST_TimerTaskHandle__ *task,
								   uint64_t time);
#endif
 ...
 * library than OpenGL. An off-screen texture of the viewport will then be drawn
 * into using OpenGL, but the final texture draw call will happen through
...
typedef void (*GHOST_XrGraphicsContextUnbindFn)(
	GHOST_ContextHandle graphics_context);
typedef void (*GHOST_XrDrawViewFn)(const struct GHOST_XrDrawViewInfo *draw_view,
								   void *customdata);

/**
 * An array of #GHOST_TXrGraphicsBinding items defining the candidate bindings
 * to use. The first available candidate will be chosen, so order defines
 * priority.
 */
...
typedef struct GHOST_XrDrawViewInfo {
	...
	/** The view that this info represents. Not necessarily the "eye index"
	 * (e.g. for quad view systems, etc). */
	char view_idx;
