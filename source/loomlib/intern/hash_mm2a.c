...
void GLU_hash_mm2a_init(HashMurMur2A *mm2, uint32_t seed)
{
	mm2->hash = seed;
	mm2->tail = 0;
	mm2->count = 0;
...
void GLU_hash_mm2a_add(HashMurMur2A *mm2, const unsigned char *data, size_t len)
{
...
void GLU_hash_mm2a_add_int(HashMurMur2A *mm2, int data)
{
	GLU_hash_mm2a_add(mm2, (const unsigned char *)&data, sizeof(data));
}

uint32_t GLU_hash_mm2a_end(HashMurMur2A *mm2)
{
	MM2A_MIX(mm2->hash, mm2->tail);
