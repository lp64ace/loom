...
GHOST_TSuccess GHOST_DisplayManager::getNumDisplays(
...
GHOST_TSuccess GHOST_DisplayManager::getNumDisplaySettings(
...
GHOST_TSuccess GHOST_DisplayManager::getDisplaySetting(
	uint8_t display, int32_t index, GHOST_DisplaySetting &setting) const
{
	...
	if (success == GHOST_kSuccess) {
		if (display < numDisplays &&
			(uint8_t(index) < m_settings[display].size())) {
...
GHOST_TSuccess GHOST_DisplayManager::findMatch(
	uint8_t display,
...
GHOST_TSuccess GHOST_DisplayManager::initializeSettings()
{
	...
	if (success == GHOST_kSuccess) {
		for (uint8_t display = 0;
			 (display < numDisplays) && (success == GHOST_kSuccess);
			 display++) {
			...
			if (success == GHOST_kSuccess) {
				...
				for (index = 0;
					 (index < numSettings) && (success == GHOST_kSuccess);
					 index++) {
