...
class IndexRange {
   ...
   public:
	...
	constexpr explicit IndexRange(size_t start, size_t size)
	...
	// Get the nth last element in the range. Asserts when the range is empty or
	// when n is negative.
	...
	/** Get the element one before the beginning. The returned value is
	 * undefined when the range is empty, and the range must start after zero
	 * already. */
	 ...
	 * the range. This invokes undefined behavior when n is negative. */
	...
	/** Returns a new IndexRange that only contains the first n elements. This
	...
	/** Returns a new IndexRange that only contains the last n elements. This
	 * invokes undefined behavior when n is negative. */
	...
	/** Move the range forward or backward within the larger array. The amount
	 * may be negative, but its absolute value cannot be greater than the
	 * existing start of the range. */
	constexpr IndexRange Shift(int64_t n) const
	{
		return IndexRange(mStart + n, mSize);
	}

	// Get read-only access to a memory buffer that contains the range as actual
	// numbers.
...
/** Split a range into three parts so that the boundaries of the middle part are
 * aligned to some power of two.
 *
 * This can be used when an algorithm can be optimized on aligned
 * indices/memory. The algorithm then needs a slow path for the beginning and
 * end, and a fast path for the aligned elements. */
AlignedIndexRanges split_index_range_by_alignment(const IndexRange range,
