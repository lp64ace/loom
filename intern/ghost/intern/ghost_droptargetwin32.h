...
class GHOST_DropTargetWin32 : public IDropTarget {
   public:
	 ...
	 * methods in IUnknown are the first entries in the VTable for every
	 * interface.
	 */
	 ...
	 * methods used in any application that can be a target for data during a
	...
	HRESULT __stdcall DragEnter(IDataObject *p_data_object,
								...
								POINTL pt,
								DWORD *pdw_effect);
	HRESULT __stdcall DragOver(DWORD grf_key_state,
							   POINTL pt,
							   DWORD *pdw_effect);
