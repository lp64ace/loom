 ...
 * scaling functions. */
typedef UINT(WINAPI *GHOST_WIN32_GetDpiForWindow)(HWND);
typedef BOOL(WINAPI *GHOST_WIN32_AdjustWindowRectExForDpi)(
...
GHOST_WindowWin32::GHOST_WindowWin32(GHOST_SystemWin32 *system,
	...
	: GHOST_Window(width, height, state, wantStereoVisual, false),
	  ...
	  m_user32(::LoadLibrary(_T ( "user32.dll" ))),
	  ...
	  m_debug_context(is_debug)
{
	...
	m_hWnd = ::CreateWindowExW(
		extended_style,					/* window extended style */
	...
	if (!setDrawingContextType(type)) {
		...
		const char *text =
			"A graphics card and driver with support for OpenGL 3.3 or higher "
			"is "
		...
		if (GetSystemMetrics(SM_CMONITORS) > 1) {
			text =
				...
				"higher is "
				"required.\n\nPlugging all monitors into your primary graphics "
				"card might resolve "
				"this issue. Installing the latest driver for your graphics "
				"card could also help.";
		}
		MessageBoxA(m_hWnd, text, title, MB_OK | MB_ICONERROR);
	...
	/* Register as drop-target. #OleInitialize(0) required first, done in
	 * GHOST_SystemWin32. */
	...
	m_directManipulationHelper = GHOST_DirectManipulationHelper::create(
		m_hWnd, getDPIHint());
}
...
GHOST_WindowWin32::~GHOST_WindowWin32()
{
	...
	if (m_hWnd) {
		/* If this window is referenced by others as parent, clear that relation
		 * or windows will free the handle while we still reference it. */
		for (GHOST_IWindow *iter_win :
			 m_system->getWindowManager()->getWindows()) {
		...
		if (m_dropTarget) {
			...
			/* Release our reference of the DropTarget and it will delete itself
			 * eventually. */
														  ...
														  DWORD dwExStyle)
{
	...
	LONG width = min(monitor.rcWork.right - monitor.rcWork.left,
					 win_rect->right - win_rect->left);
	LONG height = min(monitor.rcWork.bottom - monitor.rcWork.top,
	...
	/* With Windows 10 and newer we can adjust for chrome that differs with DPI
	 * and scale. */
	GHOST_WIN32_AdjustWindowRectExForDpi fpAdjustWindowRectExForDpi = nullptr;
	if (m_user32) {
		fpAdjustWindowRectExForDpi =
			(GHOST_WIN32_AdjustWindowRectExForDpi)::GetProcAddress(
				m_user32, "AdjustWindowRectExForDpi");
	}

	/* Adjust to allow for caption, borders, shadows, scaling, etc. Resulting
	 * values can be correctly outside of monitor bounds. NOTE: You cannot
	 * specify #WS_OVERLAPPED when calling. */
	if (fpAdjustWindowRectExForDpi) {
		UINT dpiX, dpiY;
		GetDpiForMonitor(hmonitor, MDT_EFFECTIVE_DPI, &dpiX, &dpiY);
		fpAdjustWindowRectExForDpi(
			win_rect, dwStyle & ~WS_OVERLAPPED, FALSE, dwExStyle, dpiX);
	}
	else {
		AdjustWindowRectEx(
			win_rect, dwStyle & ~WS_OVERLAPPED, FALSE, dwExStyle);
	}
...
GHOST_TSuccess GHOST_WindowWin32::setClientWidth(uint32_t width)
{
	...
	if (cBnds.getWidth() != (int32_t)width) {
		...
		success =
			::SetWindowPos(
				m_hWnd, HWND_TOP, 0, 0, cx, cy, SWP_NOMOVE | SWP_NOZORDER) ?
				GHOST_kSuccess :
				GHOST_kFailure;
	}
	else {
...
GHOST_TSuccess GHOST_WindowWin32::setClientSize(uint32_t width, uint32_t height)
{
	...
	getClientBounds(cBnds);
	if ((cBnds.getWidth() != (int32_t)width) ||
		(cBnds.getHeight() != (int32_t)height)) {
		getWindowBounds(wBnds);
		int cx = wBnds.getWidth() + width - cBnds.getWidth();
		int cy = wBnds.getHeight() + height - cBnds.getHeight();
		success =
...
GHOST_TWindowState GHOST_WindowWin32::getState() const
{
	if (::IsIconic(m_hWnd)) {
		...
		return (result & WS_CAPTION) ? GHOST_kWindowStateMaximized :
...
GHOST_TSuccess GHOST_WindowWin32::setOrder(GHOST_TWindowOrder order)
{
	...
	if (order == GHOST_kWindowOrderBottom) {
		...
		hWndToRaise = ::GetWindow(m_hWnd,
								  GW_HWNDNEXT); /* the window to raise */
	}
	else {
	...
	if (::SetWindowPos(
			m_hWnd, hWndInsertAfter, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE) ==
		FALSE) {
	...
	if (hWndToRaise &&
		::SetWindowPos(
			hWndToRaise, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE) ==
			FALSE) {
		return GHOST_kFailure;
	}
...
GHOST_TSuccess GHOST_WindowWin32::invalidate()
{
	GHOST_TSuccess success;
	if (m_hWnd) {
		success = ::InvalidateRect(m_hWnd, 0, FALSE) != 0 ? GHOST_kSuccess :
															GHOST_kFailure;
	}
	else {
...
GHOST_Context *GHOST_WindowWin32::newDrawingContext(
	GHOST_TDrawingContextType type)
{
	if (type == GHOST_kDrawingContextTypeOpenGL) {
		...
		GHOST_Context *context = new GHOST_ContextVK(
			false, m_hWnd, 1, 0, m_debug_context);
...
HCURSOR GHOST_WindowWin32::getStandardCursor(GHOST_TStandardCursor shape) const
{
	...
	switch (shape) {
		...
		case GHOST_kStandardCursorRightArrow:
			cursor = ::LoadImage(module,
								 _T ( "arrowright_cursor" ),
								 IMAGE_CURSOR,
								 cx,
								 cy,
								 flags);
		...
		case GHOST_kStandardCursorLeftArrow:
			cursor = ::LoadImage(
				module, _T ( "arrowleft_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorUpArrow:
			cursor = ::LoadImage(
				module, _T ( "arrowup_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorDownArrow:
			cursor = ::LoadImage(
				module, _T ( "arrowdown_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorVerticalSplit:
			cursor = ::LoadImage(
				module, _T ( "splitv_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorHorizontalSplit:
			cursor = ::LoadImage(
				module, _T ( "splith_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorKnife:
			cursor = ::LoadImage(
				module, _T ( "knife_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorEyedropper:
			cursor = ::LoadImage(module,
								 _T ( "eyedropper_cursor" ),
								 IMAGE_CURSOR,
								 cx,
								 cy,
								 flags);
		...
		case GHOST_kStandardCursorZoomIn:
			cursor = ::LoadImage(
				module, _T ( "zoomin_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorZoomOut:
			cursor = ::LoadImage(
				module, _T ( "zoomout_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorMove:
			cursor = ::LoadImage(
				module, _T ( "handopen_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorNSEWScroll:
			cursor = ::LoadImage(module,
								 _T ( "scrollnsew_cursor" ),
								 IMAGE_CURSOR,
								 cx,
								 cy,
								 flags);
		...
		case GHOST_kStandardCursorNSScroll:
			cursor = ::LoadImage(
				module, _T ( "scrollns_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorEWScroll:
			cursor = ::LoadImage(
				module, _T ( "scrollew_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		...
		case GHOST_kStandardCursorCrosshair:
			cursor = ::LoadImage(
				module, _T ( "cross_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break; /* Standard Cross */
		case GHOST_kStandardCursorCrosshairA:
			cursor = ::LoadImage(
				module, _T ( "crossA_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break; /* Crosshair A */
		case GHOST_kStandardCursorCrosshairB:
			cursor = ::LoadImage(
				module, _T ( "crossB_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break; /* Diagonal Crosshair B */
		case GHOST_kStandardCursorCrosshairC:
			cursor = ::LoadImage(
				module, _T ( "crossC_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break; /* Minimal Crosshair C */
		case GHOST_kStandardCursorBottomSide:
		case GHOST_kStandardCursorUpDown:
			cursor = ::LoadImage(
				module, _T ( "movens_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break; /* Double-pointed arrow pointing north and south */
		case GHOST_kStandardCursorLeftSide:
		case GHOST_kStandardCursorLeftRight:
			cursor = ::LoadImage(
				module, _T ( "moveew_cursor" ), IMAGE_CURSOR, cx, cy, flags);
		...
		case GHOST_kStandardCursorPencil:
			cursor = ::LoadImage(
				module, _T ( "pencil_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorEraser:
			cursor = ::LoadImage(
				module, _T ( "eraser_cursor" ), IMAGE_CURSOR, cx, cy, flags);
			break;
		case GHOST_kStandardCursorDestroy:
		case GHOST_kStandardCursorStop:
			cursor = ::LoadImage(
				module, _T ( "forbidden_cursor" ), IMAGE_CURSOR, cx, cy, flags);
...
void GHOST_WindowWin32::loadCursor(bool visible,
								   GHOST_TStandardCursor shape) const
{
	if (!visible) {
...
GHOST_TSuccess GHOST_WindowWin32::setWindowCursorGrab(
	GHOST_TGrabCursorMode mode)
{
	if (mode != GHOST_kGrabDisable) {
		if (mode != GHOST_kGrabNormal) {
			m_system->getCursorPosition(m_cursorGrabInitPos[0],
										m_cursorGrabInitPos[1]);
		...
		if (m_cursorGrab == GHOST_kGrabHide) {
			m_system->setCursorPosition(m_cursorGrabInitPos[0],
										m_cursorGrabInitPos[1]);
...
GHOST_TSuccess GHOST_WindowWin32::setWindowCursorShape(
...
GHOST_TSuccess GHOST_WindowWin32::hasCursorShape(
	GHOST_TStandardCursor cursorShape)
{
	...
	LPARAM lParam)
{
	...
	if (!(GetPointerPenInfoHistory(
			pointerId, &outCount, pointerPenInfo.data()))) {
		return GHOST_kFailure;
	}
	...
	for (uint32_t i = 0; i < outCount; i++) {
		...
		if (pointerPenInfo[i].penMask & PEN_MASK_PRESSURE) {
			outPointerInfo[i].tabletData.Pressure = pointerPenInfo[i].pressure /
													1024.0f;
		}
...
{
	if (m_user32) {
		GHOST_WIN32_GetDpiForWindow fpGetDpiForWindow =
			(GHOST_WIN32_GetDpiForWindow)::GetProcAddress(m_user32,
														  "GetDpiForWindow");
	...
	bool canInvertColor)
{
	...
	cols = sizeX /
		   8; /* Number of whole bytes per row (width of bitmap/mask). */
	if (sizeX % 8) {
		cols++;
	}
	...
	m_customCursor = ::CreateCursor(
		::GetModuleHandle(0), hotX, hotY, 32, 32, andData, xorData);
	if (!m_customCursor) {
		return GHOST_kFailure;
	}
...
GHOST_TSuccess GHOST_WindowWin32::setProgressBar(float progress)
{
	...
	if (m_Bar &&
		S_OK == m_Bar->SetProgressValue(m_hWnd, 10000 * progress, 10000)) {
		return GHOST_kSuccess;
	}
