...
#ifndef _WIN32_IE
#	define _WIN32_IE \
		0x0501 /* shipped before XP, so doesn't impose additional requirements \
				*/
#endif
...
#	define DEVICE_COUNT 1
	...
	devices[0].usUsage =
		0x06; /* http://msdn.microsoft.com/en-us/windows/hardware/gg487473.aspx
			   */
	...
	if (RegisterRawInputDevices(
			devices, DEVICE_COUNT, sizeof(RAWINPUTDEVICE))) {
		/* Success. */
	}
	else {
	}
	...
	: m_hasPerformanceCounter(false), m_freq(0), m_start(0), m_lfstart(0)
{
	 ...
	 * blurry scaling and enables WM_DPICHANGED to allow us to draw at proper
	 * DPI. */
...
{
	/* Hardware does not support high resolution timers. We will use
	 * GetTickCount instead then. */
...
bool GHOST_SystemWin32::processEvents(bool waitForEvent)
{
	...
	do {
		...
		while (::PeekMessageW(&msg, NULL, 0, 0, PM_REMOVE) != 0) {
			/* #TranslateMessage doesn't alter the message, and doesn't change
			 * our raw keyboard data. Needed for #MapVirtualKey or if we ever
			 * need to get chars from wm_ime_char or similar. */
		 ...
		 * without us noticing, so we need to check the event manager here to
		 * see if there are events waiting in the queue.
		 */
...
GHOST_TSuccess GHOST_SystemWin32::getModifierKeys(
...
GHOST_TSuccess GHOST_SystemWin32::getButtons(GHOST_Buttons &buttons) const
{
	 ...
	 * GetAsyncKeyState() will give back the state of the physical mouse
	 * buttons.
	 */
...
GHOST_TSuccess GHOST_SystemWin32::init()
{
	 ...
	 */
	m_hasPerformanceCounter = ::QueryPerformanceFrequency(
								  (LARGE_INTEGER *)&m_freq) == TRUE;
	if (m_hasPerformanceCounter) {
	...
	if (success) {
		...
		wc.hIcon = ::LoadIcon(wc.hInstance, _T ( "APPICON" ));
...
{
	/* #RI_KEY_BREAK doesn't work for sticky keys release, so we also check for
	 * the up message. */
	unsigned int msg = raw.data.keyboard.Message;
	*r_key_down = !(raw.data.keyboard.Flags & RI_KEY_BREAK) &&
...
/**
 * \note this function can be extended to include other exotic cases as they
 * arise.
 *
										 ...
										 short extend) const
{
	...
	if ((vKey >= '0') && (vKey <= '9')) {
		...
		switch (vKey) {
			...
			case VK_OEM_8:
			default:
				key = ((GHOST_SystemWin32 *)getSystem())
						  ->processSpecialKey(vKey, scanCode);
				break;
		}
	}
...
GHOST_EventButton *GHOST_SystemWin32::processButtonEvent(
	GHOST_TEventType type, GHOST_WindowWin32 *window, GHOST_TButton mask)
{
...
void GHOST_SystemWin32::processPointerEvent(unsigned int type,
											...
											bool &eventHandled)
{
	...
	switch (type) {
		case WM_POINTERUPDATE: {
			/* Coalesced pointer events are reverse chronological order, reorder
			 * chronologically. Only contiguous move events are coalesced. */
			for (uint32_t i = pointerInfo.size(); i-- > 0;) {
				system->pushEvent(
					new GHOST_EventCursor(pointerInfo[i].time,
		...
		case WM_POINTERDOWN: {
			/* Move cursor to point of contact because GHOST_EventButton does
			 * not include position. */
			system->pushEvent(
				new GHOST_EventCursor(pointerInfo[0].time,
									  GHOST_kEventCursorMove,
									  window,
...
GHOST_EventCursor *GHOST_SystemWin32::processCursorEvent(
	GHOST_WindowWin32 *window, const int32_t screen_co[2])
{
	...
	if (window->getCursorGrabModeIsWarp()) {
		 ...
		 * the mouse event is outdated. Identify these cases by checking if the
		 * cursor is not yet within bounds. */
		...
		/* Warp within bounds. */
		{
			...
			if (window->getCursorGrabMode() == GHOST_kGrabHide) {
				...
				/* WARNING(@campbellbarton): The current warping logic fails to
				 * warp on every event, so the box needs to small enough not to
				 * let the cursor escape the window but large enough that the
				 * cursor isn't being warped every time. If this was not the
				 * case it would be less trouble to simply warp the cursor to
				 * the center of the screen on every motion, see: D16558
				 * (alternative fix for T102346). */
				const int32_t subregion_div =
					4; /* One quarter of the region. */
				const int32_t size[2] = {bounds.getWidth(), bounds.getHeight()};
				const int32_t center[2] = {(bounds.m_l + bounds.m_r) / 2,
			...
			/* Could also clamp to screen bounds wrap with a window outside the
			 * view will fail at the moment. Use inset in case the window is at
			 * screen bounds. */
										  ...
										  LPARAM lParam)
{
	...
	while (acc >= WHEEL_DELTA) {
		system->pushEvent(
			new GHOST_EventWheel(system->getMilliSeconds(), window, direction));
		acc -= WHEEL_DELTA;
	}
												   ...
												   RAWINPUT const &raw)
{
	 ...
	 * Typically keyboard events are *not* filtered as part of GHOST's event
	 * handling. As other GHOST back-ends don't have the behavior, it's simplest
	 * not to send them through. Ideally it would be possible to check the
	 * key-map for keys that repeat but this doesn't look to be supported. */
	...
	if (!is_repeated_modifier) {
		...
		/* We can be here with !key_down if processing dead keys (diacritics).
		 * See T103119. */
		 ...
		 * won't allow diacritical composition. XXX: we are not checking return
		 * of MapVirtualKeyW for high bit set, which is what is supposed to
		 * indicate dead keys. But this is working now so approach cautiously.
		 */
		else if (MapVirtualKeyW(vk, MAPVK_VK_TO_CHAR) != 0) {
			wchar_t utf16[3] = {0};
			int r;
			/* TODO: #ToUnicodeEx can respond with up to 4 utf16 chars (only 2
			 * here). Could be up to 24 utf8 bytes. */
			if ((r = ToUnicodeEx(vk,
								 raw.data.keyboard.MakeCode,
								 state,
								 utf16,
			...
			if (!key_down) {
				/* Clear or wm_event_add_ghostevent will warn of unexpected data
				 * on key up. */
				utf8_char[0] = '\0';
			}
		}
...
GHOST_Event *GHOST_SystemWin32::processWindowSizeEvent(
	GHOST_WindowWin32 *window)
{
	GHOST_SystemWin32 *system = (GHOST_SystemWin32 *)getSystem();
	...
	/* We get WM_SIZE before we fully init. Do not dispatch before we are
	 * continuously resizing. */
											...
											LPARAM lParam)
{
	...
	if (hwnd) {
		...
		GHOST_WindowWin32 *window = (GHOST_WindowWin32 *)::GetWindowLongPtr(
			hwnd, GWLP_USERDATA);
		if (window) {
			switch (msg) {
				...
				case WM_INPUT: {
					 ...
					 * ================================================= */
				case WM_IME_NOTIFY: {
					/* Update conversion status when IME is changed or input
					 * mode is changed. */
				...
				case WM_IME_STARTCOMPOSITION: {
					...
					event = processImeEvent(GHOST_kEventImeCompositionStart,
											window,
											&ime->eventImeData);
					break;
				}
				case WM_IME_COMPOSITION: {
					...
					if (ime->eventImeData.result_len) {
						...
						eventManager->removeTypeEvents(
							GHOST_kEventImeComposition, window);
					}
					event = processImeEvent(
						GHOST_kEventImeComposition, window, &ime->eventImeData);
					break;
				}
				case WM_IME_ENDCOMPOSITION: {
					GHOST_ImeWin32 *ime = window->getImeInput();
					eventHandled = true;
					/* remove input event after end comp event, avoid redundant
					 * input */
					eventManager->removeTypeEvents(GHOST_kEventKeyDown, window);
					ime->ResetComposition(hwnd);
					ime->DestroyImeWindow(hwnd);
					event = processImeEvent(GHOST_kEventImeCompositionEnd,
											window,
											&ime->eventImeData);
					break;
				}
				 ...
				 * #TranslateMessage function. WM_CHAR contains the character
				 * code of the key that was pressed. */
				case WM_DEADCHAR:
					/* The #WM_DEADCHAR message is posted to the window with the
					 * keyboard focus when a WM_KEYUP message is translated by
					 * the #TranslateMessage function. WM_DEADCHAR specifies a
					 * character code generated by a dead key. A dead key is a
					 * key that generates a character, such as the umlaut
				 ...
				 * character code of a system dead key - that is, a dead key
				 * that is pressed while holding down the alt key. */
				case WM_SYSCHAR: {
					/* #The WM_SYSCHAR message is sent to the window with the
					 * keyboard focus when a WM_SYSCHAR message is translated by
					 * the #TranslateMessage function. WM_SYSCHAR specifies the
					 * character code of a dead key - that is, a dead key that
					 * is pressed while holding down the alt key. To prevent the
					 * sound, #DefWindowProc must be avoided by return. */
					break;
				}
				case WM_SYSCOMMAND: {
					/* The #WM_SYSCOMMAND message is sent to the window when
					 * system commands such as maximize, minimize  or close the
					 * window are triggered. Also it is sent when ALT button is
					 * press for menu. To prevent this we must return preventing
					 * #DefWindowProc.
					 *
					 * Note that the four low-order bits of the wParam parameter
					 * are used internally by the OS. To obtain the correct
					...
					switch (wParam & 0xFFF0) {
						...
						case SC_MAXIMIZE: {
							/* Don't report event as handled so that default
							 * handling occurs. */
							break;
						}
						case SC_MINIMIZE: {
				...
				case WM_POINTERUP: {
					processPointerEvent(
						msg, window, wParam, lParam, eventHandled);
					break;
				}
				case WM_POINTERLEAVE: {
					...
					/* Reset pointer pen info if pen device has left tracking
					 * range. */
				...
				case WM_MOUSEMOVE: {
					...
					const int32_t window_co[2] = {GET_X_LPARAM(lParam),
												  GET_Y_LPARAM(lParam)};
					int32_t screen_co[2];
					window->clientToScreen(UNPACK2(window_co),
										   UNPACK2(screen_co));
				...
				case WM_SETCURSOR: {
					 ...
					 * is not captured. This means we have to set the cursor
					 * shape every time the mouse moves! The DefWindowProc
					 * function uses this message to set the cursor to an arrow
					 * if it is not in the client area.
					 */
					if (LOWORD(lParam) == HTCLIENT) {
						/* Load the current cursor. */
						window->loadCursor(window->getCursorVisibility(),
										   window->getCursorShape());
				...
				case WM_NCMOUSEMOVE: {
					 ...
					 * cursor is moved within the non-client area of the window.
				...
				case WM_ACTIVATE: {
					...
					system->m_keycode_last_repeat_key = 0;
					event = processWindowEvent(LOWORD(wParam) ?
												   GHOST_kEventWindowActivate :
												   GHOST_kEventWindowDeactivate,
											   window);
					/* WARNING: Let DefWindowProc handle WM_ACTIVATE, otherwise
					 * WM_MOUSEWHEEL will not be dispatched to OUR active window
					 * if we minimize one of OUR windows. */
				...
				case WM_ENTERSIZEMOVE: {
					 ...
					 * after it enters the moving or sizing modal loop. The
					 * window enters the moving or sizing modal loop when the
					 * user clicks the window's title bar or sizing border, or
					 * when the window passes the WM_SYSCOMMAND message to the
				...
				case WM_PAINT: {
					/* An application sends the WM_PAINT message when the system
					 * or another application makes a request to paint a portion
					 * of an application's window. The message is sent when the
					 ...
					 */
					if (!window->m_inLiveResize) {
						event = processWindowEvent(GHOST_kEventWindowUpdate,
												   window);
				...
				case WM_GETMINMAXINFO: {
					 ...
					 * default maximized size and position, or its default
				...
				case WM_SIZE: {
					 ...
					 * has changed. The WM_SIZE and WM_MOVE messages are not
					 * sent if an application handles the WM_WINDOWPOSCHANGED
					 * message without calling DefWindowProc. It is more
					 * efficient to perform any move or size change processing
					 * during the WM_WINDOWPOSCHANGED message without calling
					 * DefWindowProc.
					 */
					event = processWindowSizeEvent(window);
					break;
				 ...
				 */
				case WM_MOVE: {
					/* The WM_SIZE and WM_MOVE messages are not sent if an
					 * application handles the WM_WINDOWPOSCHANGED message
					 * without calling DefWindowProc. It is more efficient to
					 * perform any move or size change processing during the
					 * WM_WINDOWPOSCHANGED message without calling
					 * DefWindowProc.
					 */
					/* See #WM_SIZE comment. */
					if (window->m_inLiveResize) {
						system->pushEvent(
							processWindowEvent(GHOST_kEventWindowMove, window));
						system->dispatchEvents();
					}
					else {
						event = processWindowEvent(GHOST_kEventWindowMove,
												   window);
					}
				...
				case WM_DPICHANGED: {
					/* The WM_DPICHANGED message is sent when the effective dots
					 * per inch (dpi) for a window has changed. The DPI is the
					 * scale factor for a window. There are multiple events that
					 * can cause the DPI to change such as when the window is
					...
					system->pushEvent(processWindowEvent(
						GHOST_kEventWindowDPIHintChanged, window));
					system->dispatchEvents();
					eventHandled = true;
				...
				case WM_KILLFOCUS: {
					 ...
					 * before it loses the keyboard focus. We want to prevent
					 * this if a window is still active and it loses focus to
					 * nowhere. */
				...
				case WM_SETTINGCHANGE: {
					/* Microsoft: "Note that some applications send this message
					 * with lParam set to NULL" */
					if ((lParam != NULL) &&
				 ...
				 * window-management function. The WM_SIZE and WM_MOVE messages
				 * are not sent if an application handles the
				 * WM_WINDOWPOSCHANGED message without calling DefWindowProc. It
				 * is more efficient to perform any move or size change
				 * processing during the WM_WINDOWPOSCHANGED message without
				 * calling DefWindowProc.
				 */
				case WM_ERASEBKGND:
				/* An application sends the WM_ERASEBKGND message when the
				 * window background must be erased (for example, when a window
				 * is resized). The message is sent to prepare an invalidated
				 ...
				 * non-client area needs to be changed to indicate an active or
				 * inactive state. */
				case WM_DESTROY:
				...
				case WM_NCDESTROY: {
					 ...
					 * non-client area is being destroyed. The DestroyWindow
				 ...
				 * size, position, or place in the Z order is about to change as
				 * a result of a call to the SetWindowPos function or another
				 ...
				 * caller. */
				case WM_ACTIVATEAPP:
				/* The WM_ACTIVATEAPP message is sent when a window belonging to
				 * a different application than the active window is about to be
				 * activated. The message is sent to the application whose
				 * window is being activated and to the application whose window
				 * is being deactivated. */
				case WM_TIMER: {
					/* The WIN32 docs say:
					 * The WM_TIMER message is posted to the installing thread's
					 * message queue when a timer expires. You can process the
					 * message by providing a WM_TIMER case in the window
					 * procedure. Otherwise, the default window procedure will
					 * call the TimerProc callback function specified in the
					 * call to the SetTimer function used to install the timer.
					 *
				...
				case DM_POINTERHITTEST: {
					/* The DM_POINTERHITTEST message is sent to a window, when
					 * pointer input is first detected, in order to determine
					 * the most probable input target for Direct Manipulation.
					 */
...
GHOST_TSuccess GHOST_SystemWin32::showMessageBox(
	const char *title,
	const char *message,
	...
	GHOST_DialogOptions dialog_options) const
{
	...
	if ( TaskDialogIndirect ( &config , &nButtonPressed , NULL , NULL ) == S_OK ) {
...
#else
	MessageBoxW(HWND_DESKTOP,
				message_16,
				title_16,
				(dialog_options & GHOST_DialogError ? MB_ICONERROR :
				 dialog_options & GHOST_DialogWarning ? MB_ICONWARNING :
														MB_ICONINFORMATION) |
					MB_OK);
#endif
...
char *GHOST_SystemWin32::getClipboard(bool selection) const
{
	...
	if (IsClipboardFormatAvailable(CF_TEXT) && OpenClipboard(NULL)) {
		...
		strncpy_s(temp_buff, len + 1, buffer, len);
		temp_buff[len] = '\0';
...
static bool getProcessName(int pid, char *buffer, int max_len)
{
	...
	if (handle) {
		GetModuleFileNameExA(handle, 0, buffer, max_len);
		result = true;
	}
	CloseHandle(handle);
	return result;
...
static bool isStartedFromCommandPrompt()
{
	...
	if (hwnd) {
		...
		/* When we're starting from a wrapper we need to compare with parent
		 * process ID. */
