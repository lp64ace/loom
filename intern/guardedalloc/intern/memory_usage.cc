...
struct alignas( 128 ) Local {
	 ...
	 * destructed, we know that the application is quitting and that we can't rely on thread 
	 * locals being available still.
	 */
	bool is_main = false;
	
	/**
	 * Number of bytes. This can be negative when e.g. one thread allocates a lot of memory, and 
	 * another frees it. It has to be atomic, because it may be accessed by other threads when the 
	 * total memory usae is counted.
	 */
	std::atomic<int64_t> mem_in_use = 0;

	/**
	 * Number of allcoated blocks. This can be negative when e.g. one thread allocates a lot of 
	 * blocks, and another deletes it. It has to be atomic, because it may be accessed by other 
	 * threads when the total memory usae is counted.
	 */
