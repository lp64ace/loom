...
GHOST_TSuccess GHOST_ContextWGL::setSwapInterval(int interval)
{
	if (epoxy_has_wgl_extension(m_hDC, "WGL_EXT_swap_control"))
		return WIN32_CHK(::wglSwapIntervalEXT(interval)) == TRUE ?
				   GHOST_kSuccess :
				   GHOST_kFailure;
	else
							   ...
							   PIXELFORMATDESCRIPTOR &preferredPFD)
{
	...
	if ((preferredPFD.dwFlags & PFD_SUPPORT_COMPOSITION) &&
									  ...
									  PIXELFORMATDESCRIPTOR &preferredPFD)
{
	...
	/* choose a pixel format using the useless Windows function in case we come
	 * up empty handed */
	...
	for (int i = 1; i <= lastPFD; i++) {
		...
		if (w > stereoWeight &&
			(preferredPFD.dwFlags & pfd.dwFlags & PFD_STEREO)) {
			stereoWeight = w;
...
/**
 * Clone a window for the purpose of creating a temporary context to initialize
 * WGL extensions. There is no generic way to clone the lpParam parameter, so
 * the caller is responsible for cloning it themselves.
 */
...
struct DummyContextWGL {
	...
	DummyContextWGL(HDC hDC, HWND hWnd, bool stereoVisual, bool needAlpha)
	{
		...
		if (!WIN32_CHK(::DescribePixelFormat(hDC,
											 dummyPixelFormat,
		...
		if (!WIN32_CHK(
				::SetPixelFormat(dummyHDC, dummyPixelFormat, &chosenPFD)))
			return;
		...
		has_WGL_ARB_pixel_format = epoxy_has_wgl_extension(
			hDC, "WGL_ARB_pixel_format");
		has_WGL_ARB_create_context = epoxy_has_wgl_extension(
			hDC, "WGL_ARB_create_context");
		has_WGL_ARB_create_context_profile = epoxy_has_wgl_extension(
			hDC, "WGL_ARB_create_context_profile");
...
#define _MAX_PIXEL_FORMATS 32
	...
	WIN32_CHK(wglChoosePixelFormatARB(m_hDC,
									  &(iAttributes[0]),
									  NULL,
									  _MAX_PIXEL_FORMATS,
									  iPixelFormats,
...
#ifdef WIN32_COMPOSITING
		if (needAlpha) {
			// scan through all pixel format to make sure one supports
			// compositing
			PIXELFORMATDESCRIPTOR pfd;
			int i;

			for (i = 0; i < nNumFormats; i++) {
				if (DescribePixelFormat(m_hDC,
										iPixelFormats[i],
										sizeof(PIXELFORMATDESCRIPTOR),
										&pfd)) {
			...
			if (i == nNumFormats) {
				fprintf(stderr,
						"Warning! Unable to find a pixel format with "
	...
	if (iPixelFormat != 0) {
		if (needAlpha) {
			...
			wglGetPixelFormatAttribivARB(
				m_hDC, iPixelFormat, 0, 1, &iQuery, &alphaBits);
			if (alphaBits == 0) {
				fprintf(stderr,
						"Warning! Unable to find a frame buffer with alpha "
						"channel.\n");
			}
		}
	}
...

	{
		...
		if (!dummy.has_WGL_ARB_create_context || ::GetPixelFormat(m_hDC) == 0) {
			...
			if (dummy.has_WGL_ARB_pixel_format)
				iPixelFormat = choose_pixel_format_arb(m_stereoVisual,
													   needAlpha);

			if (iPixelFormat == 0)
				iPixelFormat = choose_pixel_format_legacy(m_hDC,
														  dummy.preferredPFD);
			...
			if (needAlpha && chosenPFD.cAlphaBits == 0)
				fprintf(stderr,
						"Warning! Unable to find a pixel format with an alpha "
						"channel.\n");
		...
		if (dummy.has_WGL_ARB_create_context) {
			...
			if (profileMask != m_contextProfileMask)
				fprintf(stderr,
						"Warning! Ignoring untested OpenGL context profile "
						"mask bits.");
...
{
	GHOST_TSuccess success = m_hGLRC != s_sharedHGLRC || s_sharedCount == 1 ?
								 GHOST_kSuccess :
								 GHOST_kFailure;
