...
typedef bool ( *GHashCmpFP )( const void *a , const void *b );  // returns false when equal
 ...
 */
GHash *GLU_ghash_new_ex ( GHashHashFP hash_fp , GHashCmpFP cmp_fp , const char *info , unsigned int nentries_reserve );
 ...
 */
GHash *GLU_ghash_copy ( const GHash *gh , GHashKeyCopyFP keycopy_fp , GHashValCopyFP valcopy_fp );
 ...
 * \param nentrie_reserve: reserve the number of members that the hash will hold.
 * Use this to avoid resizing buckets if the size is known or can be closely approximated.
 */
 ...
 */
bool GLU_ghash_reinsert ( GHash *gh , void *key , void *val , GHashKeyFreeFP keyfree_fp , GHashValFreeFP valfree_fp );
 ...
 * \note When NULL is a valid value, use #GLU_ghash_lookup_p to differentiate a missing key
 * from a key with a NULL value. (Avoids calling #GLU_ghash_haskey before #GLU_ghash_lookup)
 */
 ...
 */
bool GLU_ghash_remove ( GHash *gh , const void *key , GHashKeyFreeFP keyfree_fp , GHashValFreeFP valfree_fp );
 ...
 * \param nentries_reserve: Optionally reserve the number of members that the hash will hold.
 */
void GLU_ghash_clear_exs ( GHash *gh , GHashKeyFreeFP keyfree_fp , GHashValFreeFP valfree_fp , unsigned int nentries_reserve );

// Wraps #GLU_ghash_clear_ex with zero entries reserved.
void GLU_ghash_clear ( GHash *gh , GHashKeyFreeFP keyfree_fp , GHashValFreeFP valfree_fp );
...
#define GHASH_ITER(gh_iter_, ghash_) \
  for (GLU_ghash_iterator_init(&gh_iter_, ghash_); GLU_ghash_iterator_done(&gh_iter_) == false; GLU_ghash_iterator_step(&gh_iter_))

#define GHASH_ITER_INDEX(gh_iter_, ghash_, i_) \
  for (GLU_ghash_iterator_init(&gh_iter_, ghash_), i_ = 0; GLU_ghash_iterator_done(&gh_iter_) == false; GLU_ghash_iterator_step(&gh_iter_), i_++)
 ...
 * \param nentries_reserve: Optionally reserve the number of members that the hash will hold.
 * Use this to avoid resizing buckets if the size is known or can be closely approximated.
 * \return  An empty GSet.
 */
GSet *GLU_gset_new_ex ( GSetHashFP hashfp , GSetCmpFP cmpfp , const char *info , unsigned int nentries_reserve );
 ...
 * \param gset: The GSet to copy.
 * \return Returns the newly allocated GSet with the same contents as the one provided.
 */
 ...
 * \param r_key The removed key.
 * \param state Used for efficient removal.
 * \return Returns true if there was something to pop, false if gset is already empty.
 */
bool GLU_gset_pop ( GSet *gs , GSetIterState *state , void **r_key );

/**
 * Removes the specified key from \a gs, returning true if the key could be removed, false otherwise.
 *
 * \param key The key we want to remove.
 * \param keyfreefp Optional function to free the key.
 * \return Returns true if the \a key was removed, false if the \a key was not found in the gset.
 */
 ...
 * \param nentries_reserve: Optionally reserve the number of members that the hash will hold.
 */
void GLU_gset_clear_ex ( GSet *gs , GSetKeyFreeFP keyfreefp , unsigned int nentries_reserve );
 ...
 * \return Returns the pointer to the key if it's found, removing it from the gset.
...
#define GSET_ITER(gs_iter_, gset_) for (GLU_gset_iterator_init(&gs_iter_, gset_); GLU_gset_iterator_done(&gs_iter_) == false; GLU_gset_iterator_step(&gs_iter_))

#define GSET_ITER_INDEX(gs_iter_, gset_, i_) for (GLU_gset_iterator_init(&gs_iter_, gset_), i_ = 0; GLU_gset_iterator_done(&gs_iter_) == false; GLU_gset_iterator_step(&gs_iter_), i_++)
...
/** 
 * This function implements the widely used `djb`. The 32 bit unsigned hash value
 * starts at 5381 and for each byte 'c' in the string, is updated: `hash = hash * 33 + c`.
...
#define GLU_ghashutil_inthash_v4(key) (GLU_ghashutil_uinthash_v4((const unsigned int *)key))
...
#define GLU_ghashutil_inthash_v4_murmur(key) (GLU_ghashutil_uinthash_v4_murmur((const unsigned int *)key))
#define GLU_ghashutil_inthash_v4_p_murmur ((GSetHashFP)GLU_ghashutil_uinthash_v4_murmur)
#define GLU_ghashutil_uinthash_v4_p_murmur ((GSetHashFP)GLU_ghashutil_uinthash_v4_murmur)
