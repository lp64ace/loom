 ...
 * In short: everything that Blender needed from GLUT to run on all its
 * supported operating systems and some extra's. This includes :
 *
 * - Time(r) management.
...
class GHOST_ISystem {
   public:
	 ...
	 * loaded. \param background: loading the system for background rendering
	 * (no visible windows). \return An indication of success.
	 */
	...
	/**
	 * Returns a pointer to the one and only system (nil if it hasn't been
	 * created). \return A pointer to the system.
	 */
	static GHOST_ISystem *getSystem();
	/**
	 * Return an identifier for the one and only system.
	 * \warning while it may be tempting this should never be used to check for
	 * supported features, in that case, the GHOST API should be extended to
	 * query capabilities. This is needed for X11/WAYLAND on Unix, without this
	 * - there is no convenient way for users to check if WAYLAND or XWAYLAND
	 * are in use since they are dynamically selected at startup. When
	 * dynamically switching between X11/WAYLAND is removed, this function can
	 * go too.
	 */
   ...
   public:
	...
	/**
	 * Installs a timer.
	 * Note that, on most operating systems, messages need to be processed in
	 * order for the timer callbacks to be invoked. \param delay: The time to
	 * wait for the first call to the timerProc (in milliseconds). \param
	 * interval: The interval between calls to the timerProc (in milliseconds).
	 * \param timerProc: The callback invoked when the interval expires.
	 * \param userData: Placeholder for user data.
	 * \return A timer task (0 if timer task installation failed).
	 */
	virtual GHOST_ITimerTask *installTimer(
		uint64_t delay,
		uint64_t interval,
	 ...
	 */
	virtual void getMainDisplayDimensions(uint32_t &width,
	 ...
	 * full-screen). \param is_dialog: Stay on top of parent window, no icon in
	 * taskbar, can't be minimized. \param parentWindow: Parent (embedder)
	 * window \return The new window (or 0 if creation failed).
	 */
	 ...
	 * \return Indication of success.
	 */
	virtual GHOST_TSuccess removeEventConsumer(
		GHOST_IEventConsumer *consumer) = 0;
	...
	/**
	 * Returns the current location of the cursor (location in window
	 * coordinates) \param x: The x-coordinate of the cursor. \param y: The
	 * y-coordinate of the cursor. \return Indication of success.
	 */
	virtual GHOST_TSuccess getCursorPositionClientRelative(
		const GHOST_IWindow *window, int32_t &x, int32_t &y) const = 0;
	/**
	 ...
	 * \return Indication of success.
	 */
	virtual GHOST_TSuccess setCursorPositionClientRelative(
		GHOST_IWindow *window, int32_t x, int32_t y) = 0;

	/**
	 * Returns the current location of the cursor (location in screen
	 * coordinates) \param x: The x-coordinate of the cursor. \param y: The
	 * y-coordinate of the cursor. \return Indication of success.
	 */
	 ...
	 * device being moved). \param x: The x-coordinate of the cursor. \param y:
	 * The y-coordinate of the cursor. \return Indication of success.
	 */
	 ...
	 * \return Indication of success.
	 */
	virtual GHOST_TSuccess getModifierKeyState(GHOST_TModifierKey mask,
											   bool &isDown) const = 0;
	 ...
	 * \return Indication of success.
	 */
	virtual GHOST_TSuccess getButtonState(GHOST_TButton mask,
										  bool &isDown) const = 0;
