...
class GHOST_WindowWin32 : public GHOST_Window {
   public:
	 ...
	 * \param dwStyle: The Window Style of the window whose required size is to
	 * be calculated. \param dwExStyle: The Extended Window Style of the window.
	 */
	...
	/**
	 * Set or Release mouse capture (should be called for any real button
	 * press).
	 *
	...
	/**
	 * Inform the window that it has lost mouse capture, called in response to
	 * native window system messages (WA_INACTIVE, WM_CAPTURECHANGED) or if
	 * ReleaseCapture() is explicitly called (for new window creation).
	 */
	 ...
	 * \param outPointerInfo: Storage to return resulting GHOST_PointerInfoWin32
	 * structs. \param wParam: WPARAM of the event. \param lParam: LPARAM of the
	 * event. \return True if #outPointerInfo was updated.
	 */
	GHOST_TSuccess getPointerInfo(
   ...
   private:
	...
	/**
	 * Flag if an operator grabs the mouse with #WM_cursor_grab_enable,
	 * #WM_cursor_grab_disable Multiple grabs must be released with a single
	 * un-grab.
	 */
	bool m_hasGrabMouse;
