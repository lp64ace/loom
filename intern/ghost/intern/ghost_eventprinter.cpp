...
bool GHOST_EventPrinter::processEvent(GHOST_IEvent *event)
{
	...
	std::cout << "GHOST_EventPrinter::processEvent, time: "
			  << (int32_t)event->getTime() << ", type: ";
	switch (event->getType()) {
		...
		case GHOST_kEventButtonUp: {
			...
			std::cout << "GHOST_kEventCursorButtonUp, button: "
					  << buttonData->button;
		} break;
		case GHOST_kEventButtonDown: {
			GHOST_TEventButtonData *buttonData =
				(GHOST_TEventButtonData *)((GHOST_IEvent *)event)->getData();
			std::cout << "GHOST_kEventButtonDown, button: "
					  << buttonData->button;
		} break;
		...
		case GHOST_kEventCursorMove: {
			...
			std::cout << "GHOST_kEventCursorMove, (x,y): (" << cursorData->x
					  << "," << cursorData->y << ")";
		} break;

		case GHOST_kEventKeyUp: {
			GHOST_TEventKeyData *keyData =
				(GHOST_TEventKeyData *)((GHOST_IEvent *)event)->getData();
			char str[32] = {'\0'};
		...
		case GHOST_kEventDraggingEntered: {
			...
			std::cout << " mouse at x=" << dragnDropData->x
					  << " y=" << dragnDropData->y;
		} break;
		...
		case GHOST_kEventDraggingExited: {
			GHOST_TEventDragnDropData *dragnDropData =
				(GHOST_TEventDragnDropData *)((GHOST_IEvent *)event)->getData();
			std::cout << "GHOST_kEventDraggingExited, dragged object type : "
					  << dragnDropData->dataType;
		} break;
		...
		case GHOST_kEventOpenMainFile: {
			...
			if (eventData) {
				std::cout << "GHOST_kEventOpenMainFile for path : "
						  << (char *)eventData;
			}
			else {
				std::cout
