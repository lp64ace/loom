		...
		switch (error & 0x0000FFFF) {
			...
			case ERROR_INCOMPATIBLE_DEVICE_CONTEXTS_ARB:
				msg = _T(
					...
					"different drivers or possibly on different graphics "
					"adapters.\n");
				break;
...
#	ifdef WITH_GLEW_ES
			case ERROR_INCOMPATIBLE_AFFINITY_MASKS_NV:
				msg =
					_T ( "The device context(s) and rendering context have non-matching affinity masks.\n" );
				break;

			case ERROR_MISSING_AFFINITY_MASK_NV:
				msg =
					_T ( "The rendering context does not have an affinity mask set.\n" );
				break;
#	endif
...
#	else
		_ftprintf(
			stderr, _T ( "Win32 Error# (%lu): %s" ), (unsigned long)error, msg);
#	endif
