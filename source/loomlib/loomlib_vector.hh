...
template<
	...
	typename _Tp,
	/** The number of values that can be stored in this vector, without doing a
	 * heap allocation. Sometimes it makes sense to increase this value a lot.
	 * The memory in the inline buffer is not initialized when it is not needed.
	 *
	 * When _Tpis large, the small buffer optimization is disabled by default to
	 * avoid large unexpected allocations on the stack. It can still be enabled
	 * explicitly though. */
	size_t _InlineBufferCapacity = default_inline_buffer_capacity(sizeof(_Tp)),
	/** The allocator used by this vector. Should rarely be changed, except when
	 * you don't want that MEM_* is used internally. */
...
class Vector {
   ...
   private:
	/** Use pointers instead of storing the size explicitly. This reduces the
	 * number of instructions in `append`.
	 *
	...
	/** Be a friend with other vector instantiations. This is necessary to
	 * implement some memory management logic. */
	template<typename Other_Tp,
			 size_t OtherInlineBufferCapacity,
   ...
   public:
	...
	Vector(NoExceptConstructor, _Allocator allocator = {}) noexcept
		: Vector(allocator)
	{
	}
	 ...
	 * If _Tpis trivially constructible, the elements in the vector are not
	 * touched. */
	...
	Vector(size_t size, const _Tp &value, _Allocator allocator = {})
		: Vector(allocator)
	{
		this->Resize(size, value);
	}

	// Create a vector from a span. The values in the vector are copy
	// constructed.
	template<typename U, LOOM_ENABLE_IF((std::is_convertible_v<U, _Tp>))>
	Vector(Span<U> values, _Allocator allocator = {})
	...
	/** Create a vector that contains copies of the values in the initialized
	 * list. This allows you to write code like: Vector<int> vec = {3, 4, 5}; */
	template<typename U, LOOM_ENABLE_IF((std::is_convertible_v<U, _Tp>))>
	...
	template<typename U,
			 size_t N,
			 LOOM_ENABLE_IF((std::is_convertible_v<U, _Tp>))>
	Vector(const std::array<U, N> &values) : Vector(Span(values))
	{
	}

	template<typename InputIt,
			 /* This constructor should not be called with e.g. Vector(3, 10),
			  * because that is expected to produce the vector (10, 10, 10). */
			 LOOM_ENABLE_IF((!std::is_convertible_v<InputIt, int>))>
	Vector(InputIt first, InputIt last, _Allocator allocator = {})
		: Vector(NoExceptConstructor(), allocator)
	{
	...
	/** Create a copy of another vector. The other vector will not be changed.
	 * If the other vector has less than InlineBufferCapacity elements, no
	 * allocation will be made. */
	Vector(const Vector &other) : Vector(other.AsSpan(), other.mAllocator)
	{
	}

	/** Create a copy of a vector with a different InlineBufferCapacity. This
	 * needs to be handled separately, so that the other one is a valid copy
	 * constructor. */
	template<size_t OtherInlineBufferCapacity>
	Vector(const Vector<_Tp, OtherInlineBufferCapacity, _Allocator> &other)
		: Vector(other.AsSpan(), other.mAllocator)
	{
	}

	/** Steal the elements from another vector. This does not do an allocation.
	 * The other vector will have zero elements afterwards. */
	template<size_t OtherInlineBufferCapacity>
	Vector(Vector<_Tp, OtherInlineBufferCapacity, _Allocator> &&other) noexcept(
		std::is_nothrow_move_constructible_v<_Tp>)
		: Vector(NoExceptConstructor(), other.mAllocator)
	{
		...
		if (other.IsInline()) {
			if (size <= _InlineBufferCapacity) {
				...
				mBegin = static_cast<_Tp *>(mAllocator.allocate(
					sizeof(_Tp) * static_cast<size_t>(capacity),
					alignof(_Tp),
					AT));
	...
	// Get the value at the given index. This invokes undefined behavior when
	 ...
	 * If new_size is smaller than the old size, the elements at the end of the
	 * vector are destructed. If new_size is larger than the old size, the new
	 * elements at the end are default constructed. If _Tpis trivially
	 * constructible, the memory is not touched by this function. */
	 ...
	 * If new_size is smaller than the old size, the elements at the end of the
	 * vector are destructed. If new_size is larger than the old size, the new
	 * elements will be copy constructed from the given value. */
	 ...
	 * All existing elements are destructed, and not copied if the data must be
	 * reallocated. */
	...
	/** Append the value to the vector and return the index that can be used to
	...
	size_t AppendAndGetIndexAs(ForwardValue &&...value)
	{
	...
	/** Append the value if it is not yet in the vector. This has to do a linear
	 * search to check if the value is in the vector. Therefore, this should
	 * only be called when it is known that the vector is small. */
	...
	/** Append the value and assume that vector has enough memory reserved. This
	 * invokes undefined behavior when not enough capacity has been reserved
	...
	/** Enlarges the size of the internal buffer that is considered to be
	 * initialized. This invokes undefined behavior when the new size is larger
	 * than the capacity. The method can be useful when you want to call
	 * constructors in the vector yourself. This should only be done in very
	 * rare cases and has to be justified every time. */
	...
	/** Adds all elements from the array that are not already in the vector.
	 * This is an expensive operation when the vector is large, but can be very
	 * cheap when it is known that the vector is small. */
	...
	/** Extend the vector without bounds checking. It is assumed that enough
	 * memory has been reserved beforehand. Only use this in performance
	 * critical code. */
	...
	/** Insert elements into the vector at the specified position. This has a
	 * running time of O(n) where n is the number of values that have to be
	 * moved. Undefined behavior is invoked when the insert position is out of
	 * bounds. */
	...
	template<typename InputIt>
	void Insert(const _Tp *insert_position, InputIt first, InputIt last)
	{
	...
	template<typename InputIt>
	void Insert(const size_t insert_index, InputIt first, InputIt last)
	{
		...
		for (size_t i = 0; i < move_amount; i++) {
			...
			try {
				new (static_cast<void *>(mBegin + dst_index))
					_Tp(std::move(mBegin[src_index]));
			}
			catch (...) {
		...
		try {
			std::uninitialized_copy_n(
				first, insert_amount, mBegin + insert_index);
		}
		catch (...) {
	...
	/** Insert values at the beginning of the vector. The has to move all the
	 * other elements, so it has a linear running time. */
	...
	/** Destructs the last element and decreases the size by one. This invokes
	 * undefined behavior when the vector is empty. */
	void RemoveLast()
	{
	...
	/** Remove the last element from the vector and return it. This invokes
	 * undefined behavior when the vector is empty. This is similar to
	 * std::vector::pop_back. */
	_Tp PopLast()
	{
		LOOM_assert(!this->IsEmpty());
	...
	/** Delete any element in the vector. The empty space will be filled by the
	 * previously last element. This takes O(1) time. */
	...
	/** Finds the first occurrence of the value, removes it and copies the last
	 * element to the hole in the vector. This takes O(n) time. */
	...
	/** Remove the element at the given index and move all values coming after
	 * it one towards the front. This takes O(n) time. If the order is not
	 * important, remove_and_reorder should be used instead.
	 *
	...
	/** Remove a contiguous chunk of elements and move all values coming after
	 * it towards the front. This takes O(n) time.
	 *
	 * This is similar to std::vector::erase. */
	void Remove(const size_t start_index, const size_t amount)
	{
		...
		for (size_t i = 0; i < move_amount; i++) {
			mBegin[start_index + i] = std::move(
				mBegin[start_index + amount + i]);
		}
	...
	/** Do a linear search to find the value in the vector and return the found
	 * index. This invokes undefined behavior when the value is not in the
	 * vector. */
	...
	/** Get the current capacity of the vector, i.e. the maximum number of
	 * elements the vector can hold, before it has to reallocate. */
   ...
   private:
	...
	void ReallocToAtLeast(const size_t min_capacity)
	{
		...
		/* At least double the size of the previous allocation. Otherwise
		 * consecutive calls to grow can cause a reallocation every time even
		 * though min_capacity only increments. */
		const size_t min_new_capacity = this->Capacity() * 2;

		const size_t new_capacity = std::max(min_capacity, min_new_capacity);
		const size_t size = this->Size();

		_Tp *new_array = static_cast<_Tp *>(mAllocator.allocate(
			static_cast<size_t>(new_capacity) * sizeof(_Tp), alignof(_Tp), AT));
		try {
