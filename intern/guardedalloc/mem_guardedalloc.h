...
extern void *(*MEM_reallocN_id)(void *vmemh, size_t len, const char *str);

/**
 * A variant of realloc which zeros new bytes
 */
extern void *(*MEM_recallocN_id)(void *vmemh, size_t len, const char *str);
 ...
 */
extern void *(*MEM_callocN)(size_t len, const char *str);
 ...
 * pointer to it is stored ! */
extern void *(*MEM_calloc_arrayN)(size_t len, size_t size, const char *str);

/**
 * Allocate a block of memory of size len, with tag name str. The
 * name must be a static, because only a pointer to it is stored !
 */
extern void *(*MEM_mallocN)(size_t len, const char *str);
 ...
 * name must be a static, because only a pointer to it is stored !
 */
extern void *(*MEM_malloc_arrayN)(size_t len, size_t size, const char *str);
 ...
 * vmemh. Only affects the guarded allocator. The name must be a static string,
 * because only a pointer to it is stored!
 *
 * Handy when debugging leaking memory allocated by some often called, generic
 * function with a unspecific name. A caller with more info can set a more
 * specific name, and see which call to the generic function allocates the
 * leaking memory.
 */
...
/**
 * This should be called as early as possible in the program. When it has been
 * called, information about memory leaks will be printed on exit.
 */
...
/**
 * When this has been called and memory leaks have been detected, the process
 * will have an exit code that indicates failure. This can be used for when
 * checking for memory leaks with automated tests.
 */
 ...
 * allocated blocks.
 *
 * NOTE: The switch between allocator types can only happen before any
 ...
 */
template<typename T>
inline T *MEM_cnew_array(const size_t length, const char *allocation_name)
{
	static_assert(std::is_trivial_v<T>,
				  "For non-trivial types, MEM_new should be used.");
	return static_cast<T *>(
		MEM_calloc_arrayN(length, sizeof(T), allocation_name));
}
...
#	define MEM_CXX_CLASS_ALLOC_FUNCS(_id) \
	   public: \
		...
		/* This is the matching delete operator to the placement-new operator \
		 * above. Both parameters will have the same value. Without this, we \
		 * get the warning C4291 on windows. */ \
