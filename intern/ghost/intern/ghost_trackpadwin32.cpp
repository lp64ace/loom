...
GHOST_DirectManipulationHelper::GHOST_DirectManipulationHelper(
	HWND hWnd,
	Microsoft::WRL::ComPtr<IDirectManipulationManager>
		directManipulationManager,
	Microsoft::WRL::ComPtr<IDirectManipulationUpdateManager>
		directManipulationUpdateManager,
	Microsoft::WRL::ComPtr<IDirectManipulationViewport>
	...
	: m_hWnd(hWnd),
	  ...
	  m_directManipulationViewportHandlerCookie(
		  directManipulationViewportHandlerCookie),
	  m_isScrollDirectionInverted(isScrollDirectionInverted)
{
}

GHOST_DirectManipulationHelper *GHOST_DirectManipulationHelper::create(
	HWND hWnd, uint16_t dpi)
{
#define DM_CHECK_RESULT_AND_EXIT_EARLY(hr, failMessage) \
	...
	DM_CHECK_RESULT_AND_EXIT_EARLY(hr,
								   "DirectManipulationManager create failed\n");

	/* Since we want to use fake viewport, we need to send fake updates to
	 * UpdateManager. */
	Microsoft::WRL::ComPtr<IDirectManipulationUpdateManager>
		directManipulationUpdateManager;
	...
	DM_CHECK_RESULT_AND_EXIT_EARLY(
		hr, "Viewport set ActivateConfiguration failed\n");
	...
	/* We receive Direct Manipulation transform updates in
	 * IDirectManipulationViewportEventHandler callbacks. */
	Microsoft::WRL::ComPtr<GHOST_DirectManipulationViewportEventHandler>
		directManipulationEventHandler =
			Microsoft::WRL::Make<GHOST_DirectManipulationViewportEventHandler>(
				dpi);
	DWORD directManipulationViewportHandlerCookie;
	directManipulationViewport->AddEventHandler(
		hWnd,
		directManipulationEventHandler.Get(),
		&directManipulationViewportHandlerCookie);
	...
	DM_CHECK_RESULT_AND_EXIT_EARLY(
		hr, "DirectManipulationManager activate failed\n");

	hr = directManipulationViewport->Enable();
	DM_CHECK_RESULT_AND_EXIT_EARLY(hr, "Viewport enable failed\n");

	directManipulationEventHandler->resetViewport(
		directManipulationViewport.Get());

	bool isScrollDirectionInverted = getScrollDirectionFromReg();

	auto instance = new GHOST_DirectManipulationHelper(
		hWnd,
		directManipulationManager,
		directManipulationUpdateManager,
		directManipulationViewport,
...
bool GHOST_DirectManipulationHelper::getScrollDirectionFromReg()
{
	...
	HRESULT hr = HRESULT_FROM_WIN32(RegGetValueW(
		HKEY_CURRENT_USER,
...
void GHOST_DirectManipulationHelper::registerScrollDirectionChangeListener()
{
	...
	if (!m_scrollDirectionRegKey) {
		HRESULT hr = HRESULT_FROM_WIN32(
			RegOpenKeyExW(HKEY_CURRENT_USER,
						  L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Preci"
						  L"sionTouchPad\\",
						  0,
						  KEY_NOTIFY,
	...
	HRESULT hr = HRESULT_FROM_WIN32(
		RegNotifyChangeKeyValue(m_scrollDirectionRegKey,
								true,
...
{
	[[maybe_unused]] HRESULT hr = m_directManipulationViewport->SetContact(
		pointerId);
...
{
	if (m_directManipulationEventHandler->dm_status ==
			DIRECTMANIPULATION_RUNNING ||
		m_directManipulationEventHandler->dm_status ==
			DIRECTMANIPULATION_INERTIA) {
		[[maybe_unused]] HRESULT hr = m_directManipulationUpdateManager->Update(
			nullptr);
	}
}
...
{
	GHOST_TTrackpadInfo result =
		m_directManipulationEventHandler->accumulated_values;
...
GHOST_DirectManipulationHelper::~GHOST_DirectManipulationHelper()
{
	...
	hr = m_directManipulationViewport->RemoveEventHandler(
		m_directManipulationViewportHandlerCookie);
...
GHOST_DirectManipulationViewportEventHandler::
	...
	: accumulated_values({0, 0, 0}),
	  dpi(dpi),
	  dm_status(DIRECTMANIPULATION_BUILDING)
{
}
	...
	IDirectManipulationViewport *viewport)
{
	if (gesture_state != GESTURE_NONE) {
		[[maybe_unused]] HRESULT hr = viewport->ZoomToRect(
			0.0f, 0.0f, 10000.0f, 10000.0f, FALSE);
	}
	...
	DIRECTMANIPULATION_STATUS previous)
{
	...
	if (previous == DIRECTMANIPULATION_ENABLED ||
	...
	IDirectManipulationViewport *viewport, IDirectManipulationContent *content)
{
	 ...
	 * at the same time because XY transform values become nonsensical during
	 * pinch gesture.
	 *  2. GHOST requires delta values for events while DM provides
	 * transformation matrix of the current gesture.
	 *  3. GHOST events accept integer values while DM values are non-integer.
	 *     Truncated fractional parts are accumulated and accounted for in
	 * following updates.
	 */
