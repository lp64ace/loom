...
class GHOST_SystemWin32 : public GHOST_System {
   ...
   public:
	/**
	 * This method converts performance counter measurements into milliseconds
	 * since the start of the system process. \return The number of milliseconds
	 * since the start of the system process.
	 */
   ...
   public:
	 ...
	 * full-screen). \param parentWindow: Parent window. \return The new window
	 * (or 0 if creation failed).
	 */
   ...
   public:
	 ...
	 * queue. Called by GHOST_DropTargetWin32 class. \param eventType: The type
	 * of drag'n'drop event \param draggedObjectType: The type object concerned
	 ...
	 */
	static GHOST_TSuccess pushDragDropEvent(
   ...
   protected:
	 ...
	 * \param lParam: The lParam from the wndproc.
	 */
	static void processWheelEvent(GHOST_WindowWin32 *window,
								  WPARAM wParam,
								  LPARAM lParam);

	/**
	 * Creates a key event and updates the key data stored locally
	 * (m_modifierKeys). In most cases this is a straightforward conversion of
	 * key codes. For the modifier keys however, we want to distinguish left and
	 * right keys. \param window: The window receiving the event (the active
	 * window). \param raw: RawInput structure with detailed info about the key
	 * event.
	 */
	 ...
	 */
	static LRESULT WINAPI s_wndProc(HWND hwnd,
									UINT msg,
									WPARAM wParam,
									LPARAM lParam);
	...
	/** The virtual-key code (VKey) of the last press event. Used to detect
	 * repeat events. */
...
inline void GHOST_SystemWin32::handleKeyboardChange(void)
{
	...
	for (m_hasAltGr = false, i = 32; i < 256; ++i) {
		...
		/* `s == -1` means no key that translates passed char code high byte
		 * contains shift state. bit 2 Control pressed, bit 4 `Alt` pressed if
		 * both are pressed, we have `AltGr` key-combination on key-layout. */
