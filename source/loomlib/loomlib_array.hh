...
template<
	...
	/**
	 * The allocator used by this array. Should rarely be changed, except when you don't want
	 * that MEM_* functions are used internally.
	 */
...
public:
	...
	template<typename U , LOOM_ENABLE_IF ( ( std::is_convertible_v<U , _Tp> ) )> Array ( Span<U> values , _Allocator allocator = {} ) : Array ( allocator ) {
	...
	// Create a new array that contains copies of all values.
	template<typename U , LOOM_ENABLE_IF ( ( std::is_convertible_v<U , _Tp> ) )> Array ( const std::initializer_list<U> &values , _Allocator allocator = {} ) : Array ( Span<U> ( values ) , allocator ) {
	}

	Array ( const std::initializer_list<_Tp> &values , _Allocator allocator = {} ) : Array ( Span<_Tp> ( values ) , allocator ) {
	}

	/** Create a new array with the given size. All values will be default constructed. For
	 * trivial types like int, default construction does nothing.
	 *
	 * We might want another version of this in the future, that does not do default
	 * construction even for non-trivial types. This should not be the default though, because
	 * one can easily mess up when dealing with uninitialized memory.
	 */
	explicit Array ( int64_t size , _Allocator allocator = {} ) : Array ( NoExceptConstructor ( ) , allocator ) {
		mData = this->GetBufferForSize ( size );
		default_construct_n ( mData , size );
		mSize = size;
	}

	/** Create a new array with the given size. All values will be initialized by copying the
	 * given default. */
	Array ( int64_t size , const _Tp &value , _Allocator allocator = {} ) : Array ( NoExceptConstructor ( ) , allocator ) {
		LOOM_assert ( size >= 0 );
		mData = this->GetBufferForSize ( size );
	...
	/** Create a new array with uninitialized elements. The caller is responsible for
	 * constructing the elements. Moving, copying or destructing an Array with uninitialized
	 * elements invokes undefined behavior.
	 *
	 * This should be used very rarely. Note, that the normal size-constructor also does not
	 * initialize the elements when _Tp is trivially constructible. Therefore, it only makes
	 * sense to use this with non trivially constructible types.
	 *
	 * Usage:
	 *  Array<std::string> my_strings(10, NoInitialization()); */
	Array ( int64_t size , NoInitialization , _Allocator allocator = {} ) : Array ( NoExceptConstructor ( ) , allocator ) {
		LOOM_assert ( size >= 0 );
		mData = this->GetBufferForSize ( size );
		mSize = size;
	}
	...
	Array ( Array &&other ) noexcept( std::is_nothrow_move_constructible_v<_Tp> ) : Array ( NoExceptConstructor ( ) , other.mAllocator ) {
	...
	Array &operator=( Array &&other ) noexcept( std::is_nothrow_move_constructible_v<_Tp> ) {
	...
	/** Sets the size to zero. This should only be used when you have manually destructed all
	 * elements in the array beforehand. Use with care. */
	...
	/** Get the value of the InlineBufferCapacity template argument. This is the number of
	 * elements that can be stored without doing an allocation. */
	static size_t InlineBufferCapacity ( ) {
		return _InlineBufferCapacity;
	}

	/** Destruct values and create a new array of the given size. The values in the new array
...
private:
	...
	_Tp *Allocate ( size_t size ) {
		return static_cast< _Tp * >( mAllocator.allocate ( static_cast< size_t >( size ) * sizeof ( _Tp ) , alignof( _Tp ) , AT ) );
	}
